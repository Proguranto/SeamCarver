...
public class DoubleMapMinPQ<T> implements ExtrinsicMinPQ<T> {
    ...
    public T removeMin() {
        ...
        double minPriority = priorityToItem.firstKey();     // Get the lowest priority value.
        Set<T> itemsWithMinPriority = priorityToItem.get(minPriority);      // Get the set of items that have the priority value.
        T item = firstOf(itemsWithMinPriority);     // Get the first item of that set^.
        itemsWithMinPriority.remove(item);      // Remove the item from the set, will update the priorityToItem because of pass by reference.
        if (itemsWithMinPriority.isEmpty()) {       // Check if the set is now empty.
            priorityToItem.remove(minPriority);     // If it's empty remove key-value pair from priorityToItem.
        }
        itemToPriority.remove(item);    // Remove the item from itemToPriority, which uses the hashmap.
        return item;
    }
